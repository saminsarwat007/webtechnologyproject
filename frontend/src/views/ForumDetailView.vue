...
const editErrors = reactive({ title: '', content: '' })
...
async function toggleLike () {
  if (!post.value || post.value.is_deleted) return
  try {
...
async function deleteComment (id) {
  try {
    await api.delete(`/forums/${postId.value}/comments/${id}`)
...
function validateEdit () {
  ...
  else if (editForm.content.length > 5000) editErrors.content = 'Content must be 5 000 chars or fewer.'
...
async function deletePost () {
  try {
    const { data } = await api.delete(`/forums/${postId.value}`)
    const mode = data?.data?.mode
    if (mode === 'soft') {
      toast.info('Post soft-deleted (comments preserved).')
      await loadPost()
    } else {
...
const confirmCopy = computed(() => {
  ...
  if (confirmAction.value.type === 'post') {
    ...
    return {
      title: hasComments ? 'Soft-delete this post?' : 'Delete this post?',
      body:  hasComments
        ? 'This post has comments, so its content will be cleared but the thread will remain visible.'
        : 'This post has no comments and will be permanently removed.'
    }
  }
    ...
    <div v-else class="space-y-6">
      ...
      <article class="card p-6">
        ...
        <template v-if="!editing">
          ...
          <div v-if="(isOwner || auth.isAdmin) && !post.is_deleted"
            ...
            <button class="btn-danger !py-1.5 !px-3 text-sm" @click="askDeletePost">
              {{ comments.length > 0 ? 'Soft-delete' : 'Delete' }}
            </button>
          </div>
        </template>
        ...
        <template v-else>
          ...
          <form @submit.prevent="saveEdit" novalidate class="space-y-4">
            ...
            <div>
              <label class="label" for="edit-tag">Label</label>
              <input id="edit-tag" v-model="editForm.tag" class="input"
                     maxlength="60" placeholder="e.g. Interview Tips, Company Review, General" />
              <p class="mt-1 text-xs text-slate-400">Leave blank to keep as 'General'</p>
            </div>
      ...
      <section class="space-y-4">
        ...
        <form v-if="auth.isStudent && !post.is_deleted"
              class="card p-4" @submit.prevent="submitComment">
          <textarea v-model="commentText" rows="3" class="input" maxlength="2000"
                    placeholder="Add a comment…" />
          <div class="flex justify-between items-center mt-2">
            <span class="text-xs text-slate-400">{{ commentText.length }} / 2 000</span>
